...
async function getMembership(userId, skupinaId) {
  const { rows } = await getPool().query(
    `SELECT * FROM clanstvo WHERE uporabnik_id = $1 AND skupina_id = $2 AND status = 'approved'`,
    [userId, skupinaId]
  );
  return rows[0] || null;
}

// Call from a handler: `const m = await requireRole(res, user.id, skupinaId, ['admin']); if (!m) return;`
...
async function hasAvailableSeat(skupinaId) {
  ...
  const { rows: countRows } = await pool.query(
    `SELECT count(*)::int AS n FROM clanstvo WHERE skupina_id = $1 AND status = 'approved'`,
    [skupinaId]
  );
