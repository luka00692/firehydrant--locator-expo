...
- [ ] **Consolidate API routes.** The Hobby plan caps a deployment at 12 serverless functions. `api/vehicles/[id]` was already folded into `api/groups/[id]/vehicles.js` (dispatches PATCH/DELETE via a `?vehicleId=` query param) since the vehicles feature was silently broken in production otherwise — that freed a slot, which now excludes `health.js` instead (ops-only, not called by the frontend). Still excluded via `.vercelignore`: `checkout`, `webhooks/stripe`, `cron/resync`, `geocode`, `groups/[id]/requests`, and `memberships/[id]` (and `vercel.json`'s `crons` array is emptied since it pointed at the excluded resync route). Fold the rest in the same way, or upgrade the Vercel account to Pro and remove `.vercelignore` + restore the crons entry.
